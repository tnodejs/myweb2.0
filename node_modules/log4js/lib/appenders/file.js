...
function fileAppender (file, layout, logSize, numBackups) {
  ...
  if(!openFiles[file]){
  	var logFile = openTheStream(file, logSize, numBackups);
  	 openFiles[file] = logFile;
  } else {
  	logFile = openFiles[file];
  }
...
function closeOpenFiles(){
	for(var fileName in openFiles){
		var dataArr = fileName.split("/");
		var fileTime = dataArr[dataArr.length-2];
		console.log(parseInt(getDateTime()),parseInt(fileTime));
		if((parseInt(getDateTime()) - parseInt(fileTime))>2){
